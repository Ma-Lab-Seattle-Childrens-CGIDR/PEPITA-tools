...
def analyze_diamond(model_a, model_b, model_combo):
    ...
    return (
        os.path.join(
            LOG_DIR, f"{model_a.cocktail}+{model_b.cocktail}_isoboles_{uniq_str}.png"
        ),
        max_x,
        max_y,
    )
            ...
            if i not in blacklist:
                for j, element_j in enumerate(array):
                    if (
                        ...
                        and util.equalsish(element, element_j, delta=tolerance)
                    ):
                        blacklist.append(j)
...
def _get_neo_model(debug=1):
    ...
    if _neo_model == None:
        _neo_model = _get_model(
            os.path.join(util.get_here(), "examples/neo_data.csv"), debug
        )
    return _neo_model
